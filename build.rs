...
fn main() {
    ...
    let java_home = env::var("JAVA_HOME").expect("JAVA_HOME env variable is not set");

    let java_include_dir = Path::new(&java_home).join("include");

    let target = env::var("TARGET").expect("target env var is not set");
...
fn rust_swig_expand(from: &Path, out: &Path) -> Result<(), String> {
    ...
    let swig_gen = rust_swig::Generator::new(LanguageConfig::JavaConfig(JavaConfig::new(
        Path::new("java").join("src").join("com").join("binding").join("rust"),
        "com.binding.rust".into(),
    )));
